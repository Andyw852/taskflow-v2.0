...
def adaptive_parallel_tags():
    """按宿主机返回需要覆盖的 INCAR 并行参数，避免模板里硬编码 NCORE/KPAR。

    - 3090（VASP OpenACC GPU 版）：每 GPU 1 个 MPI rank，必须 NCORE=1、KPAR=1
      （NCORE>1 在 OpenACC 版会串行掉或用不满，实测慢 ~45×）。
    - CPU 集群（jzzn/a800）：保持模板默认（模板里已有按核数设计的 NCORE/KPAR），
      返回空 dict 表示不动。
    主机识别用 VASP GPU 版二进制目录是否存在，不硬编码主机名。
    """
    if os.path.isdir("/home/wangchaoyue852/software/vasp.6.6.0/bin"):
