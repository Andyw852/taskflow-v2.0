...
#
# 推完把远端目录填进 kl-mace-cpu 的全局 step.conf：
#   tf -tt kl-mace-cpu -p <材料> conf --set params.MACE_MODEL_DIR=<远端目录>
set -euo pipefail
...
echo
echo "接着设："
echo "  tf -tt kl-mace-cpu -p <材料> conf --set params.MACE_MODEL_DIR=$DEST"
echo "  tf -tt kl-mace-cpu -p <材料> conf --set params.MACE_MODEL=$(basename "${FILES[0]}")"
