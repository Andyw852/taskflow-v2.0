...
- 应变：3D → 等比体积缩放 `VOL_FACTORS`（**默认 7 档 0.94~1.06**——B0 是 required 闸，
  3 点定 E0/B0/V0 零自由度、能量误差 1:1 进 B0（GaAs 实测 B0 −41%）；7 档给 B0 抗噪。
  想省帧可项目级改 3 点，但 B0 闸会零抗噪、误差 1:1 传递，见 §9.1 ①）；2D → 面内双轴应变
...
VOL_FACTORS 各档的未位移超胞（默认 7 个）——既是 EOS 验收的 DFT 基准点（§9.2 #8），
