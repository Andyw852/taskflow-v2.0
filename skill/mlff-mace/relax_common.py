...
try:  # [MLFF] 接受 mlff-mace 的全局 step.conf 键（声明但不消费，跨步骤三层合并用）
    from mlff_common import SHARED_PARAM_SPEC as _MLFF_SHARED  # noqa: E402
except Exception:
    _MLFF_SHARED = {}
...
# [MLFF] mlff-mace 的全局参数也写在同一份 templates/step.conf 里（三层合并会带进
# step1），声明才不会被 stepconf 当未知键打死。DIMENSION / ENCUT_OVERRIDE 在
# apply_step_params 里真正生效，其余键由 step2~9 的 gen 脚本消费。
for _k, _v in (_MLFF_SHARED or {}).items():
    if _k.upper() not in CONF_SPEC:
        CONF_SPEC[_k.upper()] = (None, _v[1])
...
def load_step_params(step_name=None):
    ...
    # [MLFF] DIMENSION / ENCUT_OVERRIDE 越早生效越好：resolve_dimension 在
    # apply_step_params 之前就会跑，晚了 step.conf 的维度覆盖不生效。
    _g = globals()
    _v = _g["STEP_PARAMS"].get("DIMENSION")
    if _v in ("2d", "3d"):
        _g["DIMENSION"] = str(_v).lower()
        print("[..] step.conf 覆盖 DIMENSION = %s" % _g["DIMENSION"])
    _v = _g["STEP_PARAMS"].get("ENCUT_OVERRIDE")
    if _v:
        try:
            _g["MANUAL_ENCUT"] = int(float(_v))
            print("[..] step.conf 覆盖 ENCUT = %d eV（ENCUT_OVERRIDE）" % _g["MANUAL_ENCUT"])
        except ValueError:
            sys.exit("[ERROR] step.conf 的 ENCUT_OVERRIDE=%r 不是数字" % _v)
...
def main():
    ...
    # [MLFF] retry 语义：gen 时清掉上次作业的阶段标记。.sN.done 只表示「跑过」、
    # 不表示「收敛过」——上次 FAIL 后重投若不清理，run_relax.sh 会把所有段判成
    # 「已完成，跳过」，作业 0 秒“完成”、压强闸原样挂掉。gen 只在本步无作业时
    # 执行（do_submit 先 kill_if_queued），所以这里清理不会误伤在跑作业。
