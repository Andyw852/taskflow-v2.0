...
def main():
    ...
    if os.path.exists(incar_src):
        ...
        # ---- 偶极修正：按维度自适应（不写死 IDIPOL）----
        # 2D：真空在某轴 -> 开 LDIPOL + IDIPOL 指向真空轴；LVHAR 输出 LOCPOT 供真空对齐
        # 3D：无真空 -> 不加偶极（周期方向加偶极势无物理意义、易不收敛）；
        #     LVHAR 仍保留，方便需要时输出静电势
        try:
            _lat_v, _pos_v = read_lattice(struct)
            _vac_ax = detect_vacuum_axis(_lat_v, _pos_v, vacuum_min=8.0)  # 0/1/2 或 None
        except ValueError:
            _vac_ax = None   # 多真空方向(1D/0D) 或读取异常：不加偶极，交给上游维度检查处理
        incar_set["LVHAR"] = ".TRUE."
        if dim == "2d" and _vac_ax is not None:
            incar_set["LDIPOL"] = ".TRUE."
            incar_set["IDIPOL"] = str(_vac_ax + 1)          # 0/1/2 -> 1/2/3
            print("[..] 偶极修正：2D，真空轴=%s，IDIPOL=%d，已开 LDIPOL"
                  % ("abc"[_vac_ax], _vac_ax + 1), file=sys.stderr)
        else:
            incar_set.pop("LDIPOL", None)
            incar_set.pop("IDIPOL", None)
            print("[..] 偶极修正：%s，不加 LDIPOL/IDIPOL（仅 LVHAR 输出 LOCPOT）"
                  % ("3D 体相" if dim == "3d" else "真空轴未确定"), file=sys.stderr)
